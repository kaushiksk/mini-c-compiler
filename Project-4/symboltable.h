...
uint32_t myhash( char *lexeme )
{
	...
	*/
	for ( myhash = i = 0; i < strlen(lexeme); ++i ) {
        myhash += lexeme[i];
        myhash += ( myhash << 10 );
        myhash ^= ( myhash >> 6 );
    }
	...
	return myhash % HASH_TABLE_SIZE; // return an index in [0, HASH_TABLE_SIZE)
}
...
entry_t* insert( entry_t** hash_table_ptr, char* lexeme, int value, int data_type)
{
	...
	idx = myhash( lexeme ); // Get the index for this lexeme based on the hash function
